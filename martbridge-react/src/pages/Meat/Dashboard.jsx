...
const MeatDashboard = () => {
    ...
    const { userId: meatId, logout } = useAuth('meatId', '/meat/register');
    ...
    const fetchLabors = async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/labor/owner/${meatId}`);
            const data = await res.json();
            setLabors(data);
        } catch (err) {
    ...
    const fetchIncomingOrders = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/api/store/incoming-orders/${meatId}?date=${today}`);
            const data = await res.json();
    ...
    const fetchLinkedHotels = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/store/linked-hotels/${meatId}`);
            const data = await res.json();
    ...
    const handleSendInvite = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/store/invite-hotel`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ hotelName: inviteHotelName, storeId: meatId, storeType: "meat" })
            });

            if (res.ok) {
                const baseUrl = window.location.origin;
                const link = `${baseUrl}/hotel/register?storeId=${meatId}&storeType=meat&hotelName=${encodeURIComponent(inviteHotelName)}`;
    ...
    return (
        <div className="dashboard-page">
            ...
            {showProfilePopup && (
                <div className="overlay-blur" onClick={() => setShowProfilePopup(false)}>
                    <div className="profile-popup" onClick={e => e.stopPropagation()}>
                        ...
                        <button className="edit-btn" onClick={() => navigate('/meat/profile')}>Edit Profile</button>
            ...
            <div className="dash-sections">
                <div className="dash-card" onClick={() => navigate('/meat/set-prices')}>
                    <h3>📊 Set Today's Prices</h3>
                    <p>Update daily meat prices</p>
                </div>
                <div className="dash-card" onClick={() => navigate('/meat/orders')}>
                    <h3>📥 Incoming Orders ({incomingOrders.length})</h3>
                    <p>Orders from connected hotels</p>
                </div>
                
                <div className="dash-card labor-card" onClick={() => navigate('/meat/labor')}>
                    <h3>👷 Labor Card</h3>
                    <p>{labors.length} Workers</p>
                    <div className="summary-val">₹{totalMonthlySalary}</div>
                </div>

                <div className="dash-card" onClick={() => navigate('/meat/hotels')}>
                    <h3>🏨 Connected Hotels ({linkedHotels.length})</h3>
                    <p>View all hotels linked to your store</p>
                </div>
                <div className="dash-card" onClick={() => navigate('/meat/payments')}>
                    <h3>💳 View Payments</h3>
            ...
            <style jsx="true">{`
                .dashboard-page {
                    min-height: 100vh;
                    background: url('https://images.unsplash.com/photo-1607623814075-e51df1bdc82f') no-repeat center/cover;
                    position: relative;
                    color: white;
                    display: flex;
